...
for category in categories:
    ...
    for current_store in category:
        ...
        if(len(query_result)>0):
            ...
            y = df['qta'].to_numpy()
            X = df[['day_of_month','day_of_week','day_of_year','week_n','unit_price','flag_off','rag_soc']].to_numpy()
            ...
            mlp_gs = MLPRegressor(early_stopping=True)
            parameter_space = {
            ...
            t = time.time()
            clf = GridSearchCV(mlp_gs,parameter_space, cv=5, n_jobs=-1,refit=True,scoring='explained_variance')
